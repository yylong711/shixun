...
class SentimentClassifier:
    def __init__(self, filename='/Users/fanjialiang2401/Desktop/项目实训/VENV_shixun/shixun/emotion_analyze/corpus/train.csv'):
    ...
    def save_model(self, filename='/Users/fanjialiang2401/Desktop/项目实训/VENV_shixun/shixun/emotion_analyze/model/model.pickle'):
        with codecs.open(filename, 'wb') as f:
            pickle.dump(self.model, f)

    def train(self):
            self.model = LinearSVC(random_state=3)
            self.model.fit(self.X_train, self.y_train)
            self.save_model()
            print('Accuracy: ' + str(round(self.model.score(self.X_val, self.y_val), 2)))

    def predict(self, sentence):
        word_list = jieba.cut(sentence, True)
        vec = self.preprocessor.sentence2vec(word_list)
        vecc=np.array(vec).reshape(1,-1)
        # print(vecc)
        return self.model.predict(vecc)

    def predict_test_set(self, sentences, pos_file='/Users/fanjialiang2401/Desktop/项目实训/VENV_shixun/shixun/emotion_analyze/test/pos_test.txt', neg_file='/Users/fanjialiang2401/Desktop/项目实训/VENV_shixun/shixun/emotion_analyze/test/neg_test.txt'):
        pos_set = []
        neg_set = []
    ...
    def cal_precision(self):
        true=0
        # opener_1=open('true_neg_test','w')
        # opener_2=open('true_pos_test','w')
        with open('corpus/neg.txt')as opener:
            lines=opener.readlines()
            all=len(lines)
            print('所有的数据:{}'.format(all))
            for line in lines :
                word_list=jieba.cut(line,False)
                result=self.predict(word_list=word_list)
                if result==-1:
                    true+=1
                    # opener_1.write(line)
            print('消极的准确率为:{}'.format(true/all))
            # positive
        with open('corpus/pos.txt')as opener:
            lines=opener.readlines()
            all=len(lines)
            true=0
            for line in lines :
...
#
# setence='不太好用'
# result=classifier.predict(sentence=setence)
# print(result)
# # classifier.cal_precision()
